  ...
  `;
    ...
    const buildUI = (): void => {
        ...
        if (!document.querySelector('.sc_id')) {
            const schoolLink = document.querySelector<HTMLAnchorElement>('.school-link a.school-link-name');
            if (schoolLink && window.hasOwnProperty('APP_CONFIG') && (window as any).APP_CONFIG.school) {
